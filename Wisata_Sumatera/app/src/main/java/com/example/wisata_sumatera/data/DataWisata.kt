package com.example.wisata_sumatera.data

import com.example.wisata_sumatera.R

object DataWisata {
    val wisataTitle = arrayOf(
    ...
    val wisataLokasi = arrayOf(
        "Sido Muliyo, Kec. Nisam Antara, Kab. Aceh Utara.",
        "Jl. Sultan Iskandar Muda, Sukaramai, Kec. Baiturrahman, Kota Banda Aceh.",
        "Mon Ikeun, Kec. Lhoknga, Kab. Aceh Besar.",
        "Jl. Brigjend Katamso No.66, A U R, Kec. Medan Maimun, Kota Medan.",
        "Kab. Simalungun, Kab. Tobasa (Toba Samosir), Kab. Tapanuli Utara, Kab. Humbang Hasundutan, Kab. Dairi, Kab. Karo dan Kab. Samosir.",
        "Pengambaten, Kec. Merek, Kab. Karo.",
    ...
    val wisataSynopsis = arrayOf(
        "Air terjun kembar dengan ketinggian 75 meter ini menjadi daya tarik yang tidak akan bisa ditolak oleh siapapun. Untuk bisa mengunjunginya, Anda hanya perlu menempuh jarak sekitar 30 menit saja dari Lhokseumawe. Keindahan yang dihadirkan akan menjadi obat manjur untuk menghempaskan seluruh penat. Anda bisa berendam di kolam alami yang terbentuk di bawah air terjun, alam sekitar yang masih asri juga akan membuat Anda betah berlama lama.",
        "Tsunami yang pernah menerjang Aceh di tahun 2004 memang menyisakan duka yang begitu mendalam. Dan untuk mengenang para korban dibuatlah bangunan bernama museum Tsunami tahun 2009. Wisata di Aceh ini bisa Anda kunjungi ketika Anda berniat untuk jelajah Sumatera dari ujung. Begitu memasuki museum Anda akan dibawa kembali ke masa berat saat itu. Karena selain bisa menjumpai foto korban Anda juga bisa merasakan alat simulasi elektronik untuk gempa bumi.",
        "Aceh juga merupakan wilayah yang dekat dengan laut, dan Anda bisa menemukan pantai cantik yang dinamakan Pantai Lhoknga. Pantai satu ini tak hanya memiliki pasir putih yang lembut, namun juga memiliki air yang bersih kebiruan. Ombaknya yang tinggi sangat cocok untuk melakukan selancar jika Anda bosan berjemur di tepi pantai. Ketika sore tiba, Anda juga berkesempatan untuk menikmati cantiknya mentari yang berpulang.",
        "Melanjutkan perjalanan menuju Sumatera Utara, Anda harus mengunjungi Istana Maimun Medan yang begitu kondang. Memiliki perpaduan arsitektur Melayu, Italia, Spanyol dan Islam membuat bangunan bersejarah ini begitu unik. Sudah ada sejak tahun 1946, istana ini saat ini dihuni oleh ahli waris dari kerajaan Deli. Anda bisa menelusuri sejarah hingga berburu foto foto cantik untuk di unggah di media sosial.",
        "Siapa yang tidak mengenal Danau Toba? Danau Megah satu ini menjadi favorit wisatawan baik lokal maupun asing yang berkunjung ke tanah Sumatera. Danau yang terbentuk karena aktivitas vulkanik ini menawarkan keindahan yang begitu memukau. Alamnya yang masih terjaga akan membuat Anda betah berlama lama. Bahkan mungkin Anda enggan untuk pulang ke rumah karena suasananya yang begitu mendukung. Fasilitas di tempat wisata ini juga sudah sangat memadai.",
        "Air terjun satu ini didapuk sebagai air terjun yang paling tinggi di Indonesia. Keindahannya tidak bisa diragukan lagi, wajar jika air terjun satu ini menjadi destinasi wisata yang begitu favorit dan terkenal. Berada di kota Pisau, Anda bisa menjangkaunya dengan menggunakan kendaraan pribadi. Mengunjungi destinasi wisata satu ini, jangan sampai terlewat untuk mengambil foto dengan air terjun sebagai latar belakangnya.",
    ...
    val listWisata : ArrayList<WisataClass>
        get() {
            val list = arrayListOf<WisataClass>()
            for (position in wisataTitle.indices){
                val wisata = WisataClass()
                wisata.titleClass = wisataTitle[position]
                wisata.posterClass = wisataPoster[position]
                wisata.lokasiClass = wisataLokasi[position]
                wisata.synopsisClass = wisataSynopsis[position]
